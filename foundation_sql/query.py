...
class SQLQueryDecorator:
    ...
    def __call__(self, func: Callable) -> Callable:
        ...
        prompt_generator = SQLPromptGenerator(fn_spec, template_name, self.system_prompt, self.schema)
        ...
        def _parse_result(result_data: Any):
            if fn_spec.wrapper == 'list':
                ...
                if hasattr(result_data, "count") and callable(getattr(result_data, "count")):
                    try:
                        return int(result_data.count())
                    except Exception:
                        pass
                # 2) List of rows
